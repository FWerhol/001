(function () {
    ...
    /* Аура курсора: сетка + зелёный ореол следуют за указателем (body::before / ::after) */
    if (!reduceMotion && window.matchMedia("(hover: hover) and (pointer: fine)").matches) {
        ...
        var ty = cy;
        var auraRaf = 0;
        var auraChasing = false;

        function setCursorCss(x, y) {
            root.style.setProperty("--cursor-x", Math.round(x) + "px");
            root.style.setProperty("--cursor-y", Math.round(y) + "px");
        }
        ...
        function auraFrame() {
            ...
            var dy = ty - cy;
            if (dx * dx + dy * dy > 0.25) {
                auraRaf = window.requestAnimationFrame(auraFrame);
            } else {
                auraChasing = false;
                setCursorCss(tx, ty);
                cx = tx;
        ...
        document.addEventListener("visibilitychange", function () {
            if (document.hidden && auraRaf) {
                window.cancelAnimationFrame(auraRaf);
                auraRaf = 0;
                auraChasing = false;
            }
        });
